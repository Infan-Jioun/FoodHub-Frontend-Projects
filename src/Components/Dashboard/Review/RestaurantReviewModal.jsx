...
const RestaurantReviewModal = ({ open, onClose, payment }) => {
    ...
    const checkIfReviewed = async () => {
      try {
        const res = await axiosSecure.get(
          `/restaurant/${encodeURIComponent(restaurantName)}/review/check?email=${encodeURIComponent(user.email)}`
        );
        setAlreadyReviewed(res.data?.reviewed || false);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await axiosSecure.patch(`/restaurant/${encodeURIComponent(restaurantName)}/review`, {
        email: user.email,
      ...
      // interceptor unwrap er por: res.data = { modified: true/false, review }
  ...
  return (
    ...
    >
      ...
      <DialogBody className="space-y-5 px-6 py-4">
        ...
        <textarea
          className={`w-full border rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#ff1818] focus:border-transparent ${alreadyReviewed
      ...
      <DialogFooter className="border-t border-gray-200 px-6 py-3">
        ...
        {!alreadyReviewed && (
          <Button
            ...
            className={`bg-[#ff1818] hover:bg-[#e60000] text-white shadow-md ${rating === 0 || submitting ? "opacity-70 cursor-not-allowed" : ""
              }`}
