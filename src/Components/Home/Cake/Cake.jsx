...
const FoodModal = ({ food, open, handleOpen, handleAddFood }) => {
  ...
  const [selectedOption, setSelectedOption] = useState(
    food?.category?.toLowerCase() === "Cake" ? "8" : "full"
  );

  const handleSubmit = () => {
    const foodToAdd = {
      ...food,
      ...(food?.category?.toLowerCase() === "Cake"
        ? { variation: `${selectedOption} inch` }
        : { portion: selectedOption }),
      price: food?.category?.toLowerCase() === "Cake"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4 px-6 lg:px-4">
        {CakeFoods.length > 0 ? (
          ...
          <div className="col-span-full text-center py-10">
            <p className="text-gray-500 text-lg">No Cake items available.</p>
            <Link
              to="/restaurants"
